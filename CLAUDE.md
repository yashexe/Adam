...
14. **The residual dependency hit on the pipeline's first live day**
   [DIAGNOSED 2026-09-03, FIX REVERTED same day] — the first unattended
   run spent its whole daily budget on seven companies, all catch-all or
   firewalled, all parked with no address: exactly item 12's gap, showing
   up immediately rather than eventually. Verifiers answer "does this
   mailbox exist"; the real need was a second *source* of addresses,
   since Hunter's is dead until the 11th. Apollo's person-enrichment
   endpoint looked like that source and was built as the resolution
   ladder's last rung — then a real key hit an immediate 403: "not
   included in your Free plan... even with a master key." Every
   third-party source consulted before building said otherwise; none of
   them was Apollo's own docs. Removed rather than left inert (`git log`
   for the code, which lived for under an hour). The residual dependency
   from item 12 stands, unresolved: catch-all and firewalled domains
   still need Hunter's roster, or a paid data source, or a human pick in
   the review UI. Full account, including the correction: `docs/decisions.md`.
