...
| Eligibility rules | **implemented** | `qualify/eligibility.py` — full-time only, no frontend-titled roles. Tested against the title alone, with parentheticals stripped and full-stack titles exempt, so "Full-stack Engineer (React frontend, Python backend)" still qualifies. Removes 14 of 200 postings in a 7-day window |
