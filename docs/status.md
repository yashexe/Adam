...
| Execution model | **decided + implemented, unattended since 2026-09-03** | Claude Code-orchestrated on this Mac. `bin/tick.py` at :02:30 of every five-minute slot (LaunchAgent, installed by hand; offset from the Pi's :00 poll so its read cannot lock the poll's commit) queues new postings and fires one bounded headless run of the `outreach` skill; `outreach/unattended.py` owns the watermark, queue, budgets (3/run, 8/day) and retry. Both human gates unchanged: picks in the review UI, sends in Gmail |
...
| Resume/profile parsing | **done, by hand** | `qualify/profile.py`, written from `Yash_Bhavsar_Resume_08192026.pdf`, re-aligned with `_08282026` on 2026-09-02 (PROFILE.md is what the drafter reads; it now carries the rewritten numbers and the gated pipeline story). Instaply's `parser.py` deliberately not revived: one resume, four fields, no pipeline needed. All 46 skills resolve against `taxonomy.py` |
...
| Job descriptions | **implemented** | `qualify/boards.py` — fetched from the public Ashby/Greenhouse/Lever board APIs plus Workable's per-posting feed endpoint (the tracker stores no description), cached 24h. Lever/Workable added 2026-08-29 when the tracker started feeding them — before that their rows were silently dropped as "no longer on board" (docs/tracker-upstream-2026-08-30.md) |
...
| Slate resolution | **implemented** | `outreach_run.py verify-slate` → `outreach/verify.py resolve_slate`. Resolves an address per slate candidate (domain pattern, then Hunter's per-address roster), one cached domain-search per company, verification credits spent only until the first deliverable candidate. Advisory — finalize re-verifies the chosen contact. Since 2026-09-02 candidates with no Hunter pattern or roster entry get the keyless rung (`probe_patterns`, source `probe`): the domain's own server says which conventional address exists; partial-name hits are `risky` with the namesake caveat |
| Verify step (email check) | **implemented and live** | `outreach/verify.py`. Hunter email-verifier + domain-search pattern corroboration, cached per address (and per domain since 2026-08-26), degrades to `unverified` on any failure. `HUNTER_API_KEY` is set and exercised against the real API. Since 2026-09-02 the mailbox check is a provider chain with a keyless SMTP probe in front (`_verify_via_smtp`, benchmarked against Hunter's own labels — see `docs/decisions.md`), ZeroBounce and MillionVerifier adapters checked against their live API shapes but still awaiting keys, and Hunter last; inconclusive answers are skips, never cached labels, and `outreach_run.py verifiers` reports what can answer right now. Since 2026-08-24 also cross-checks a candidate/fallback address against Hunter's per-address names (`_name_conflict`) before returning it, closing the identity gap the company-c near-miss exposed; since 2026-08-26 `resolve_address` also falls back to a direct roster match when the pattern fails or renders someone else's mailbox |
...
| Agent 2 (draft email) | **implemented** | `.claude/agents/drafter.md`. Rewritten from Yash's real sent cold emails (voice, structure, no em dashes) — see `docs/agents.md`, "Agent 2 — the voice rewrite." Held the no-AI-tooling positioning rule under direct pressure. Rewritten again 2026-08-28 around the bridge objective after the template collapse (`docs/agents.md`, "Agent 2 — the template collapse"): research digest in, single model email out, density rules delegated to the lint, Sonnet → Opus |
...
| Slates + ignore list | **implemented** | `outreach/store.py` `slates` (a researched company parked for a human pick: awaiting → approved → drafted, or dismissed) and `company_ignore` (slugs that are not companies, seeded with a recruiting marketplace). `prepare` skips both; the review UI picks and dismisses; `outreach_run.py slates / slate / ignore` by hand |
