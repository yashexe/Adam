...
## The relevance judge tracks the real market — 7 for 7
...
processes, judged those postings blind, and compared score order to how far
each process actually went:
...
| Full-Stack SWE (deployed), company-o | 87 | live — technical round scheduled |
| Forward Deployed Engineer, company-p | 85 | recruiter screen |
| Software Engineer (fintech), company-q | 58 | first round, then rejected |
| SWE II (finance), company-r | 55 | full loop, rejected at the end |
| Python Dev (staffing), company-s | 8 | recruiter screen only |

**Score order matches process-depth order seven for seven.** The four
roles the judge rated 85+ are the four that pursued me — all
forward-deployed/embedded shapes, the same shape that tops the pipeline's
ranking. The two it put below the 65 spend bar are the two that rejected
me. The instructive row is company-n: the posting demands 3+ years of
...
Caveats stated where they belong: n=7, survivorship-biased toward roles I
chose to apply to. The full tuning history — threshold selection on a
56-posting week, validation against a second 167-judgement window, and
the deletion of a weighted composite the moment measurement showed it
only subtracted from the judge — is in [`docs/qualify.md`](docs/qualify.md).
...
*Company names throughout this repo are aliased (`company-a`,
`company-m`, …). The postings, interview processes, and verification
results are real; the private parties aren't named in a public repo.*
